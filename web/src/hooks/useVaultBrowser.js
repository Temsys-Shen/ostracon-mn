...
function useVaultBrowser(connection) {
  ...
  const openDocument = useCallback(async (path) => {
    ...
    try {
      ...
      if (requestId === openRequestRef.current) setOpeningPath("");
    }
  }, [run, command]);
  ...
  return { state, folderPath, folder, tags, selectedTag, query, activeDocuments, document, openingPath, previewMarkdown, insertMarkdown, previewHtml, insertHtml, plainText, assetUrls, loading, error, loadFolder, chooseTag, search, openDocument, setDocument };
}
